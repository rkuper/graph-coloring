...
void original_recursive_largest_first() {
  int nn[MAX_DIM];
  int nn_count = 0;
...
// loop perfected processing function
  ...
  for (i = 0; i < MAX_DIM; i++) {
    if (unprocessed >= 0) {
      ...
      for (j = 0; j < MAX_DIM; j++) {
        for (k = 0; k < MAX_DIM; k++) {
          for (l = 0; l < MAX_DIM; l++) {
          	if (j <= num_nodes && colors[j] == color_number && k < nn_count) {
              if (adj_matrix[j][nn[k]] == 1) {
                nn[k] = nn[nn_count - 1];
                nn_count--; // decrease the nn_count
              }
        ...
        if (nn_count > 0) {
          ...
          for (k = 0; k < MAX_DIM; k++) { // check the i-th vertex in nn
            ...
            for (x = 0; x < MAX_DIM; x++) {
              for (m = 0; m < MAX_DIM; m++) {
								if (k < nn_count) {
                  if (x <= num_nodes && colors[x] == color_number && m <= num_nodes) {
                    if (colors[m] == 0 && scanned[m] == 0) {
                      // if k is a neighbor in common of x and tmp_
                      if (adj_matrix[x][m] == 1 && adj_matrix[tmp_y][m] == 1) {
                        temp++;
          ...
          for (k = 0; k < MAX_DIM; k++) {
            for (l = 0; l < MAX_DIM; l++) {
						    ...
						    if (k == 0 && l == 0)
						  	...
						  	if (k < nn_count && l == 0)
						  		temp = 0;
                if (k < nn_count && l <= num_nodes)
                  if (colors[l] == 0 && adj_matrix[nn[k]][l] == 1)
                    temp++;
								if (k < nn_count && l == (MAX_DIM - 1)) {
              		if (temp > max) { // if the degree of vertex nn[i] is higher than tmp_y's one
               	 	  max = temp; // assignment nn[i] to tmp_y
                		tmp_y = nn[k];
              		}
								}
					      if (k == (MAX_DIM - 1) && l == (MAX_DIM - 1)) {
                   if (max == 0) // so all the vertices have degree 0
                     y = nn[0];
                   else // exist a maximum, return it
                     y = tmp_y;
          ...
          for (k = 0; k < MAX_DIM; k++) {
            for (l = 0; l < MAX_DIM; l++) {
              // remove vertex that adjacent to the found vertex
              for (m = 0; m < MAX_DIM; m++) {
					 		  if (k <= num_nodes && colors[k] == color_number && l < nn_count) {
                	if (adj_matrix[k][nn[l]] == 1) {
                  	nn[l] = nn[nn_count - 1];
                  	nn_count--; // decrease the nn_count
                	}
