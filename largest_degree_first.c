...
// Original largest degree first
void original_largest_degree_first() {
...
// Loop perfected largest degree first
  ...
  for (int degree = MAX_DIM - 1; degree >= 0; degree--) {
    if (degree <= max_degree) {
      ...
      for (int node = MAX_DIM; node >= 0; node--) {
        ...
        // Zero out used neighbor colors
        for (int zeroed = 0; zeroed < MAX_DIM; zeroed++)
          if ((colors[node] == 0 && node <= num_nodes) && (degrees[node] >> DEGREE_OFFSET) == degree)
            used[zeroed] = 0;
